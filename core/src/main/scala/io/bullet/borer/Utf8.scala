...
object Utf8:
  ...
  def encode(value: Array[Char]): Array[Byte] =
    ...
    @tailrec def rec(si: Int, di: Int, result: Array[Byte]): Array[Byte] =
      if si < value.length then
        val c = value(si)
        if c > 0x7F then
          var codePoint = c.toInt
          var nsi       = si
          var ndi       = di
          val nresult   = if di < result.length - 3 then result else java.util.Arrays.copyOf(result, (di + 2) << 1)
          if codePoint > 0x7FF then
            if 0xD800 <= codePoint && codePoint < 0xE000 then // UTF-16 high surrogate (i.e. first of pair)
              if codePoint >= 0xDC00 then sys.error(s"Invalid surrogate pair at index $si")
              nsi += 1
              if nsi >= value.length then sys.error("Truncated UTF-16 surrogate pair at end of char array")
          ...
          val nresult = if di < result.length then result else java.util.Arrays.copyOf(result, di << 1)
          nresult(di) = c.toByte
          rec(si + 1, di + 1, nresult)
      else if di < result.length then
  ...
  def decode(bytes: Array[Byte]): Array[Char] =
    ...
    @tailrec def decode8bit(b1: Int, si: Int, di: Int): Long =
      ...
      val cp = (byteCount | 0x80) ^ (b2 & 0xC0) match
        case 1 =>
          if (b1 & 0x1E) == 0 then fail(si)
          (b1 << 6) ^ b2 ^ 0xF80
        case 2 =>
          val b3 = quad << 8 >> 24
          val c  = (b1 << 12) ^ (b2 << 6) ^ b3 ^ 0xFFFE1F80
          if (b1 == 0xE0 && (b2 & 0xE0) == 0x80) || (b3 & 0xC0) != 0x80 || ((c >> 11) == 0x1B) then fail(si)
          c
        case 3 =>
          val b3 = quad << 8 >> 24
          val b4 = quad << 16 >> 24
          val c  = (b1 << 18) ^ (b2 << 12) ^ (b3 << 6) ^ b4 ^ 0x381F80
          if (b3 & 0xC0) != 0x80 || (b4 & 0xC0) != 0x80 || c < 0x010000 || c > 0x10FFFF then fail(si)
    ...
    @tailrec def decode8(si: Int, di: Int): Array[Char] =
      if si <= sl8 then
        ...
        if nlz < 64 then // do we have an 8-bit char anywhere?
