...
object ZBase32 extends LookupBaseEncoding("z-base32", 5, "ybndrfg8ejkmcpqxot1uwisza345h769"):
  ...
  def encode(bitCount: Long, bytes: Array[Byte]) =
    ...
    if sl.toLong << 3 < bitCount then failBitCountMismatch()
    ...
    def encodeRest(bitsRemaining: Long, si: Int, di: Int): Array[Char] =
      ...
      if bitsRemaining > 5 then result(di + 1) = alphabetChars((bits << 5 >>> 59).toInt)
      if bitsRemaining > 10 then result(di + 2) = alphabetChars((bits << 10 >>> 59).toInt)
      if bitsRemaining > 15 then result(di + 3) = alphabetChars((bits << 15 >>> 59).toInt)
      if bitsRemaining > 20 then result(di + 4) = alphabetChars((bits << 20 >>> 59).toInt)
      if bitsRemaining > 25 then result(di + 5) = alphabetChars((bits << 25 >>> 59).toInt)
      if bitsRemaining > 30 then result(di + 6) = alphabetChars((bits << 30 >>> 59).toInt)
      if bitsRemaining > 35 then result(di + 7) = alphabetChars((bits << 35 >>> 59).toInt)
      result

    @tailrec def encode5(bitsRemaining: Long, si: Int, di: Int): Array[Char] =
      if bitsRemaining >= 40 then
      ...
      else if bitsRemaining > 0 then encodeRest(bitsRemaining, si, di)
      else result
  ...
  def decode(bitCount: Long, chars: Array[Char]): Array[Byte] =
    ...
    if (sl << 2) + sl < bitCount then failBitCountMismatch()
    ...
    def decodeRest(bitsRemaining: Long, si: Int, di: Int): Array[Byte] =
      ...
      if bitsRemaining > 5 then fullBytes |= decode(si + 1) << 54
      if bitsRemaining > 10 then fullBytes |= decode(si + 2) << 49
      if bitsRemaining > 15 then fullBytes |= decode(si + 3) << 44
      if bitsRemaining > 20 then fullBytes |= decode(si + 4) << 39
      if bitsRemaining > 25 then fullBytes |= decode(si + 5) << 34
      if bitsRemaining > 30 then fullBytes |= decode(si + 6) << 29
      if bitsRemaining > 35 then fullBytes |= decode(si + 7) << 24
      val bits = fullBytes & ~(-1L >>> bitsRemaining)
      dlen - di match
    ...
    @tailrec def decode8(bitsRemaining: Long, si: Int, di: Int): Array[Byte] =
      ...
      else if bitsRemaining > 0 then decodeRest(bitsRemaining, si, di)
      else result
