...
final private[borer] class ElementDeque(maxBufferSize: Int, val next: ElementDeque):
  ...
  val prependReceiver: Receiver =
    new Receiver {
      ...
      def onBoolean(value: Boolean): Unit = ret(byteBuffer.prepend2(DIS.Boolean, if value then 1 else 0))
      ...
      def onOverLong(negative: Boolean, value: Long): Unit =
        ret(byteBuffer.prepend1(DIS.OverLong) && byteBuffer.prepend9(if negative then 1 else 0, value))
  ...
  val appendReceiver: Receiver =
    new Receiver {
      ...
      def onBoolean(value: Boolean): Unit = ret(byteBuffer.append2(DIS.Boolean, if value then 1 else 0))
      ...
      def onOverLong(negative: Boolean, value: Long): Unit =
        ret(byteBuffer.append1(DIS.OverLong) && byteBuffer.append9(if negative then 1 else 0, value))
  ...
  def pull(receiver: Receiver): Int =
    ...
    shift match
      ...
      case DIS.Boolean   => receiver.onBoolean(if byteBuffer.unsafeReadByte() != 0 then true else false)
      case DIS.Int       => receiver.onInt(byteBuffer.unsafeReadQuadByte())
      case DIS.Long      => receiver.onLong(byteBuffer.unsafeReadOctaByte())

      case DIS.OverLong =>
        receiver.onOverLong(if byteBuffer.unsafeReadByte() != 0 then true else false, byteBuffer.unsafeReadOctaByte())
  ...
  def pullAll(receiver: Receiver): Unit = while nonEmpty do pull(receiver)
  ...
  def appendElementFrom(r: Reader): Int =
    ...
    def pullComplex(dataItem: Int, level: Int): Unit =
      ...
      @tailrec def pullN(remaining: Long): Unit =
        if remaining > 0 then
          ...
          if (dataItem & DataItem.Complex) != 0 then pullComplex(dataItem, level + 1)
          pullN(remaining - 1)

      @tailrec def pullUntilBreak(): Unit =
        val dataItem = r.receiveInto(appendReceiver)
        if (dataItem & DataItem.Complex) != 0 then pullComplex(dataItem, level + 1)
        if dataItem != DataItem.Break then pullUntilBreak()
      ...
      if level < 100 then
        dataItem match
          ...
          case DataItem.MapHeader =>
            ...
            if elemsToPull >= 0 then pullN(elemsToPull)
    ...
    if (first & DataItem.Complex) != 0 then pullComplex(first, 0)
    first
  ...
  private def ret(result: Boolean): Boolean = if result then true else throw new ElementDeque.Overflow
